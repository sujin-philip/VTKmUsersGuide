...
1. Also note that if the \textidentifier{FunctionInterface} is used in a
templated function or method where the type is not fully resolved, the
compiler will not register \textcode{GetParameter} as a templated method
and will fail to parse the template argument. In this case, the
\textcode{template} keyword must added before the \textcode{GetParameter}
...
a unary function whose argument is the \textidentifier{FunctionInterface}
...
\textcode{ReturnType} with an internal type named \textcode{type} that
