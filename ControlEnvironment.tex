...
on top of \vtkmcont{DataSet} to make it easer to create data sets.
...
\fix{Expecting to change the interface to Create to use Vec. Change this
  example when that is supported.}
...
\fix{Creating an explicit mesh with \textidentifier{DataSetBuilderExplicit}.}

Often it is awkward to build your own arrays and then pass them to
\textidentifier{DataSetBuilderExplicit}. There also exist an alternate
...
\fix{Creating an explicit mesh with \textidentifier{DataSetBuilderExplicitIterative}.}

\subsubsection{Add Fields}

\fix{Still need to add this!}
...
\textidentifier{CellSetSingleType} is a good representation for this data.
...
\fix{Add example for CellSetPermutation.}
