...
with an \keyterm{implicit container}.
...
defined using the \vtkmcont{ArrayContainerControlTagImplicit} tag. This tag
is templated, and the template parameter is the implicit array portal.

\vtkmlisting[ex:ImplicitArrayContainer]{Defining the container tag for an implicit array of even numbers.}{ImplicitArrayContainer.cxx}
...
VTK-m comes with some examples of implicit
containers. \vtkmcont{ArrayHandleConstant} returns the same value for every
index in the array. The constant array is useful when an algorithm that can
work on a variable field is used on a constant value.
...
\fix{\vtkmcont{ArrayHandleTransform} is not implemented in VTK-m yet.}
...
container. A transformed array, documented in
Section~\ref{sec:TransformedArrays}, provides a specific form of a derived
container. When applicable, it is much easier to implement an array
transform than a general derived container. However, if a simple transform
...
container.  For the purposes of the example in this section, let us say we
...
The first step to creating a derived container is to build an array portal
...
\vtkmlisting[ex:DerivedArrayContainer]{\textidentifier{ArrayContainerControl} for derived container of concatenated arrays.}{DerivedArrayContainer.cxx}
...
\vtkmlisting{Prototype for \protect\vtkmcontinternal{ArrayTransfer}.}{ArrayTransferPrototype.cxx}

The \vtkmcontinternal{ArrayTransfer} specialization must define the
following items.
\begin{description}
\item[\textcode{ValueType}] A \textcode{typedef} of the type for each item
  in the array. This is the same type as the first template argument.
...
\item[\textcode{GetNumberOfValues}] A method that returns the number of
  ...
  called.
\item[\textcode{LoadDataForInput}] There are actually two versions of this
  overloaded method. The first form takes a portal of type
  \textcode{PortalControl} and the second takes an instance of
  \textidentifier{ArrayContainerControl}. Both methods are expected to
  allocate enough space in the execution environment and copy the given
  data to that array. In both cases allocated array can later be accessed
  via the and \textcode{GetPortalConstExecution} method. The data is
  assumed to be read-only. In derived arrays, it is common to implement
  only one version and throw an error in the other and enforce that only
  the implemented form is called in practice. This is done my forcing the
  \textidentifier{ArrayHandle} to be constructed either with an array portal
  (which will lead to the first form being called) or constructed with an
  array container (which will lead to the second form being called).
\item[\textcode{LoadDataForInPlace}] A method that takes an instance of
  \textidentifier{ArrayContainerControl}, allocates enough space in the
  execution environment, and copies the given data to that array. The
  allocated array can later be accessed via the
  \textcode{GetPortalExecution} and \textcode{GetPortalConstExecution}
...
\vtkmlisting[ex:DerivedArrayTransfer]{\textidentifier{ArrayTransfer} for derived container of concatenated arrays.}{DerivedArrayTransfer.cxx}
...
\vtkmcont{ArrayHandle} that accepts a constructed container.

\vtkmlisting{\textidentifier{ArrayHandle} for derived container of concatenated arrays.}{DerivedArrayHandle.cxx}
...
%% \section{Container Lists}
%% \label{sec:ContainerLists}

%% \index{container lists|(}
%% \index{lists!containers|(}
%% \index{tag!container lists|(}

%% As we just saw in Section~\ref{sec:ArrayHandles}, an
%% \textidentifier{ArrayHandle} is templated by both a base type for the
%% component and a container type. Sometimes we will not know specifically one
...
inconvenient or impossible to specify the element type and container at
...
accessible. Before the data can be accessed, the type and container of the
array must be established. This is usually done internally within VTK-m
when a worklet \fix{or filter?} is invoked. However, it is also possible to
query the types and cast to a concrete \textidentifier{ArrayHandle}.

\index{dynamic~array~handle!query}
You can query the component type and container type using the
\textcode{IsTypeAndContainer} method. This templated method takes an
example component type and an example container type as arguments and
returns whether the underlying array matches both types.

\vtkmlisting{Querying the component and container types of a \textidentifier{DynamicArrayHandle}.}{QueryDynamicArrayHandle.cxx}
...
component type and an example container type as arguments and returns an
...
are separate lists for component types and for container types.
...
Common container lists are defined in
\vtkmheader{vtkm/cont}{ContainerListTag.h}.  There is only one common
container distributed with VTK-m:
\textidentifier{ArrayContainerControlBasic}. A list tag containing this
type is defined as \vtkmcont{ContainerListTagBasic}.

As with other lists, it is possible to create new container type lists
using the existing type lists and the list bases from
Section~\ref{sec:BuildingListTags}.

The \vtkmheader{vtkm/cont}{ContainerListTag.h} header also defines a macro
named \vtkmmacro{VTKM\_DEFAULT\_CONTAINER\_LIST\_TAG} that defines a
default list of types to use in classes like
\textidentifier{DynamicArrayHandle}. This list can be overridden by
defining the \vtkmmacro{VTKM\_DEFAULT\_CONTAINER\_LIST\_TAG} macro
...
component types and container types. This can be used when the specific
...
\textcode{ResetContainerList}. These methods return a new object with that
...
component type or container type instead of the default. (Note that
...
Container type lists can be changed similarly.

\vtkmlisting{Specifying different container types in a \textidentifier{DynamicArrayHandle}.}{CastAndCallContainer.cxx}

Both the component type list and the container type list can be modified by
chaining these reset calls.

\vtkmlisting{Specifying both component and container types in a \textidentifier{DynamicArrayHandle}.}{CastAndCallTypeAndContainer.cxx}
...
\index{dynamic~point~coordinates|(}
\index{point~coordinates!dynamic|(}
...
\textidentifier{DynamicArrayHandle}. However, point coordinates are often
constructed as functional values or are derived and combined from other
arrays. Constructing lists of containers for
\textidentifier{DynamicArrayHandle} is problematic.

Thus, VTK-m provides a similar class named
\vtkmcont{DynamicPointCoordinates} designed to hold point coordinates of an
indeterminate type. \textidentifier{DynamicPointCoordinates} has an
interface and behavior very similar to that for
\textidentifier{DynamicArrayHandle} except instead of holding a reference
to a \textidentifier{ArrayHandle} object, it holds a reference to a
\textcode{PointCoordinates*} object. A point coordinate object holds enough
state to reproduce point coordinates. VTK-m comes with the following point
coordinate classes.

\begin{description}
\item[\vtkmcont{PointCoordinatesArray}] Holds point coordinates explicitly
  listed in an \textidentifier{ArrayHandle}. The component type of the
  array must be \vtkm{Vector3}, but the array can use any type of
  container.
\item[\vtkmcont{PointCoordinatesUniform}] Represents the point coordinates
  of a uniform rectilinear grid with a given extent, origin, and spacing.
\end{description}

\index{dynamic~point~coordinates!construct}
A \textidentifier{DynamicPointCoordinates} object is created with one of
these point coordinates objects. The following example demonstrates
creating \textidentifier{DynamicPointCoordinates} with common point
coordinate types.

\vtkmlisting{Creating \textidentifier{DynamicPointCoordinates} objects.}{CreateDynamicPointCoordinates.cxx}

\subsection{Querying and Casting}
\label{sec:DynamicPointCoordinatesQueryingAndCasting}

\index{dynamic~point~coordinates!query}
Given a \textidentifier{DynamicPointCoordinates} object, you can query the
type of point coordinates it contains using the
\textcode{IsPointCoordinateType} method. This templated method takes an
example point coordinate type as an argument and returns whether the
underlying array matches.

\vtkmlisting{Querying the point coordinate type of \textidentifier{DynamicPointCoordinates}.}{QueryDynamicPointCoordinates.cxx}

\index{dynamic~point~coordinates!cast}
Once the types of the \textidentifier{DynamicPointCoordinates} object are
known, it can be cast to a concrete point coordinates type with the
\textcode{CastToPointCoordinates} method. This templated method takes an
...
\textcode{CastToPointCoordinates} throws a \vtkmcont{ErrorControlBadValue}
exception.

\vtkmlisting{Casting \textidentifier{DynamicPointCoordinates} to concrete point coordinates.}{CastDynamicPointCoordinates.cxx}

All this said, casting to a point coordinate types is generally not very
...
The use case of \textidentifier{DynamicPointCoordinates} is to cast to some
form of \textidentifier{ArrayHandle}, which has a container that is often
complex and usually unknown. To perform this operation
\textidentifier{DynamicPointCoordinates} has a method named
\textcode{CastAndCall} that attempts to cast the point coordinates to an
array type.
...
\vtkmlisting[ex:UsingPCCastAndCall]{Operating on \textidentifier{DynamicPointCoordinates} with \textcode{CastAndCall}.}{UsingPCCastAndCall.cxx}
...
\index{tag!point coordinates lists|(}
\index{lists!point coordinates|(}
\index{point coordinates lists|(}

Common type lists for point coordinates are defined in
\vtkmheader{vtkm/cont}{PointCoordinatesListTag.h}. This header defines the
list tag \vtkmcont{PointCoordiantesListTagCommon} that contains the common
point coordinates types of \textidentifier{PointCoordinatesArray} and
\textidentifier{PointCoordinatesUniform}. This header file also defines the
single item lists \vtkmcont{PointCoordiantesListTagArray} and
\vtkmcont{PointCoordinatesListTagUniform}.
...
The \vtkmheader{vtkm/cont}{PointCoordinatesListTag.h} header also defines a
macro named \vtkmmacro{VTKM\_DEFAULT\_POINT\_COORDINATES\_LIST\_TAG} that
defines a default list of point coordinates to try in
\textidentifier{DynamicPointCoordinates}. This list can be overridden by
defining the \vtkmmacro{VTKM\_DEFAULT\_POINT\_COORDINATES\_LIST\_TAG} macro
\emph{before} any VTK-m headers are included. If included after a VTK-m
header, the list is not likely to take effect. Do not ignore compiler
warnings about the macro being redefined, which you will not get if defined
correctly.
...
\textidentifier{DynamicPointCoordinates} uses the aforementioned list to
find an appropriate cast to a concrete point coordinates class. Once such a
class is found, \textcode{CastAndCall} then delegates to this class the
task of casting to an \textidentifier{ArrayHandle}. This delegate point
coordinates class may (or may not) use a list of component types and
container types similar to those used in the \textcode{CastAndCall} of
\textidentifier{DynamicArrayHandle} (as described in
Section~\ref{sec:DynamicArrayHandleSpecifyingCastLists}).

There is a form of \textcode{CastAndCall} that accepts tags for the list of
point coordinate types, the list of component types and the list of
container types. This can be used when the specific lists are known at the
time of the call. However, when creating generic operations like the
\textcode{PrintPointCoordinates} function in
Example~\ref{ex:UsingPCCastAndCall}, passing these tags is inconvenient at
best.

To address this use case, \textidentifier{DynamicPointCoordinates} has a
set of methods named \textcode{ResetPointCoordinatesList},
\textcode{ResetTypeList}, and \textcode{ResetContainerList}. These methods
return a new object with that behaves just like a
\textidentifier{DynamicPointCoordinates} object with identical state except
that the cast and call functionality uses the specified point coordinates
type, component type, or container type instead of the default. (Note that
\textcode{PrintPointCoordinates} in Example~\ref{ex:UsingPCCastAndCall} is
templated on the type of \textidentifier{DynamicPointCoordinates}. This is
to accommodate using the objects from the \textcode{Reset*List} methods,
which have the same behavior but different type names.)

\fix{TODO: Add some examples here. It would be helpful if there were more
  point coordinates classes (which I expect in the future) to make
  compelling examples. See the end of Section
  \ref{sec:DynamicArrayHandleSpecifyingCastLists} for a list of related
  examples.}
...
\vtkmcontinternal{PointCoordinatesBase}. Second, a point coordinates class
defines a const \textcode{CastAndCall} method that accepts three templated
arguments: a functor, a list of component types, and a list of container
types. This method finds a concrete array class and calls the functor with
this array. The list of component types and list of container types may be
used or may be ignored.

By convention, point coordinate class names begin with
\textcode{PointCoordinates} and they have constructors that establish the
...
the vertex positions. \fix{We can make a much more compelling exampling
  once transform arrays are implemented.}

\vtkmlisting[ex:CustomPointCoordinates]{A custom point coordinates class.}{CustomPointCoordinates.cxx}
...
\textcode{CastAndCall} is invoked. Here is a simple example of creating
lists for the previously defined point coordinates and using them.

\vtkmlisting{Using a custom point coordinates class.}{UsingCustomPointCoordinates.cxx}
