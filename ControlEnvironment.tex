...
with an \keyterm{implicit storage}.
...
is to create a \keyterm{derived storage}. A derived storage shares
...
specialization of the templated \vtkmcontinternal{Storage}
class. The implementation for an \textidentifier{Storage} for
...
\item[\textcode{LoadDataForInput}] There are actually two versions of this
  ...
  \textidentifier{Storage}. Both methods are expected to
  ...
  array storage (which will lead to the second form being called).
\item[\textcode{LoadDataForInPlace}] A method that takes an instance of
  \textidentifier{Storage}, allocates enough space in the
...
\item[\textcode{AllocateArrayForOutput}] A method that takes an array
  storage and a size and allocates an array in the execution environment
  of the specified size. The initial memory is uninitialized and can be
  accessed via the \textcode{GetPortalExecution} method. The storage
...
\item[\vtkmcont{PointCoordinatesArray}] Holds point coordinates explicitly
  ...
  array must be \vtkm{Vec} with three components, but the array can use any
  type of storage.
...
form of \textidentifier{ArrayHandle}, which has a storage that is often
...
storage types similar to those used in the \textcode{CastAndCall} of
...
\textcode{ResetTypeList}, and \textcode{ResetStorageList}. These methods
